...
void MainWindow::connectZoom() {
  ...
  connect(ui->OGLwidget, &GLWidget::zoomChanged, this,
          &MainWindow::updateZoomValue);
}
...
void MainWindow::defaultSettings() {
  ...
  ui->OGLwidget->setProjectionStrategy(
        std::make_unique<s21::CentralProjection>());
...
void MainWindow::settingsFromConfig() {
  ...
  if (settings.value("projection") == "true") {
    ...
    ui->OGLwidget->setProjectionStrategy(
        std::make_unique<s21::ParallelProjection>());
  }
  settings.endGroup();
}
...
void MainWindow::OpenFile() {
  ...
  if (fileDialog.exec()) {
    ...
    if (fileNames.size() > 0) {
      ...
      auto res =
          ui->OGLwidget->controller_.loadModelFromFile(fileName.toStdString());
      if (res) {
        ui->statusbar->showMessage("Файл: " + fileName);
        ui->numberOfFaces->setText(
            QString::number(ui->OGLwidget->controller_.modelFacesCount()));
        ui->numberOfVerticies->setText(
            QString::number(ui->OGLwidget->controller_.modelVertexsCount()));
        ui->OGLwidget->initBuffers();
        ui->OGLwidget->controller_.Notify();
      } else {
        status error;
        error.exec();
      }
...
void isDirectoryExists(const QString &path) {
  QDir dir(path);
  if (!dir.exists()) {
    dir.mkpath(".");
  }
...
void MainWindow::saveBmp() {
  isDirectoryExists(pathProject);
  QString curTime = getCurrentTimeFormatted();
...
void MainWindow::updateZoomValue(int zoom) {
  ui->zoomSlider->setValue(zoom);
  ui->zoomText->setText(QString::number(zoom));
}
